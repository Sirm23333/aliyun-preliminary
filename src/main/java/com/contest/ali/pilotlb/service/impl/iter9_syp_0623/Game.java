...
import com.contest.ali.pilotlb.service.impl.iter9_syp_0623.model.*;
...
public class Game{
    ...
    public void gameRun(){
        ...
        for(int i = 0; i < tryCnt; ++i){
            ...
            for(int j = 0; j < apps.size() * upsetRatio; ++j){
                ...
                toPilotIdx = getBestIdx(tmpApp,4);
            ...
            while(-1 < (appIdx = randomNum.next())){
                ...
                delAppFromPilot(tmpApp , fromPilotIdx);
                // 找一个最好的pilot
                toPilotIdx = getBestIdx(tmpApp,1);
                // 放入最好的pilot
                setAppToPilot(tmpApp , toPilotIdx);
    ...
    private void preDistribution(){
        ...
        for(int i = 0; i < appsIdx.length; i++){
            if(appsIdx[i] < 0){
                setAppToPilot(apps.get(i),getBestIdx(apps.get(i),1));
            }
        }
    }
    ...
    private int getBestIdx(App app , int flag){
        ...
        for(int i = 0; i < pilots.size(); ++i){
            ...
            mems[i] += addMem;
            switch (flag){
                case 1:tmpScore = score1(mems,cons);break;
                case 3:tmpScore = score3(mems,cons);break;
                case 4:tmpScore = score4(mems);break;
                case 5:tmpScore = score5(mems);break;
                default:break;
            }
