...
import com.contest.ali.pilotlb.service.impl.iter5_syp_0616.model.Service;
...
@Slf4j
public class Iter5DataHandler implements DataHandler {
    ...
    private double mergeIdxRatio = 0.2;
...
//        printAppListFeature( GlobalContain.APP_LIST);

        // 二次合并
            GlobalContain.APP_LIST_MERGE = mergeAppByCoverSimilarity(GlobalContain.APP_LIST_MERGE,coverSimilarity,mergeIdxRatio,
                    GlobalContain.SERVICE_MEM / GlobalContain.PILOT_SUM,
     ...
     * 尝试合并cnt次
     */
    public List<App> mergeAppByCoverSimilarity(List<App> apps , double similarity , double idxRadio , long memUpper , long conUpper  ){
            Collections.sort(apps , (a1,a2)->{
                return (int) (a2.srvMem - a1.srvMem);
            });
            boolean[] merged = new boolean[apps.size()]; // merged为true  表示已经合并到其他app了
            List<App> mergedAppList = new ArrayList<>();
            int midIdx = (int) (apps.size() * idxRadio);
            for(int i = 0; i < midIdx; i++){
                mergedAppList.add(apps.get(i));
            }
            App app1 , app2;
            // 第一遍 由多到少
            for(int i = midIdx; i < apps.size(); ++i){
                app1 = apps.get(i);
                for(int j = midIdx - 1; j >= 0 ; --j){
            ...
            for(int i = apps.size() - 1; i >= midIdx; --i){
                if(merged[i])
                    continue;
                app1 = apps.get(i);
                for(int j = midIdx - 1; j >= 0 ; --j){
            ...
            for(int i = midIdx; i < merged.length;++i){
                if(!merged[i]){
                    mergedAppList.add(apps.get(i));
                }
            }
